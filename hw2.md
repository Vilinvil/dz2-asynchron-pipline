...
* (можно сделать отдельный канал для закрытия, или закрыть основной, а в основе делать проверку)Подумайте, как будет организовано завершение функции если данные конечны. Что для этого надо сделать?
...
* (когда закрывается канал)Ответ на вопрос "когда закрывается цикл по каналу" помогает в реализации ExecutePipeline
